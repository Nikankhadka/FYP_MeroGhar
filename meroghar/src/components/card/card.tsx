...
export default function Card({user,data}:props) {
    ...
    <main className="w-[98%] bg-white rounded-lg border-2 border-gray-300 hover:shadow-xl   duration-300">   
      ...
      <div className="margin-bottom-3">
        <Link href={`/rooms/${_id}`} target='_blank'>
          <img src={images![img].img_url} alt="property" className=" w-full  rounded-lg" />
        </Link>
  ...
  {!user&&  <div className=" flex px-2 items-center justify-between">
        <Wish active={false} />
      ...
      <div className="my-2 w-[95%] mx-auto">
        <p className="text-sm font-semibold">{location?.country},{location?.state},{location?.city}</p>
    ...
    {user=='admin'&&<div className="my-2 mt-4 w-[95%] mx-auto ">
      <button
            type="button"
            ...
            onClick={(e)=>{
              ...
              verify.onContent({
                onReject:async(message:string)=>{
                  const res=await verifyProperty(_id!,{isVerified:false,message});
                  if(res){
      ...
      {user=='user'&&<div className="my-2 mt-4 w-[95%] mx-auto ">
      <button
            type="button"
            className="focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800 inline-flex items-center rounded-lg bg-themeColor px-3 py-2 text-center text-sm font-medium text-white hover:bg-mainColor focus:ring-4"
          >
